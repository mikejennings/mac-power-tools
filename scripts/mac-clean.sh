...
# Deep clean system junk, caches, and unnecessary files
...
clean_path() {
    ...
    if [[ $size_before -eq 0 ]]; then
        [[ "$VERBOSE" == true ]] && print_color "$YELLOW" "  ⊘ $description - not found"
        return 0
    fi
    ...
    if [[ "$DRY_RUN" == true ]]; then
        print_color "$CYAN" "  [DRY RUN] Would clean: $description ($human_size)"
    ...
    if [[ -d "$path" ]]; then
        if rm -rf "$path" 2>/dev/null; then
            print_color "$GREEN" "  ✓ Cleaned $description ($human_size)"
            ((TOTAL_FREED += size_before))
        else
            print_color "$RED" "  ✗ Failed to clean $description"
        fi
    elif [[ -f "$path" ]]; then
...
clean_pattern() {
    ...
    if [[ $count -gt 0 ]]; then
        ...
        if [[ "$DRY_RUN" == true ]]; then
            print_color "$CYAN" "  [DRY RUN] Would clean: $count $description files ($human_size)"
        else
            print_color "$GREEN" "  ✓ Cleaned $count $description files ($human_size)"
        fi
        ((TOTAL_FREED += total_size))
    else
        [[ "$VERBOSE" == true ]] && print_color "$YELLOW" "  ⊘ No $description files found"
    fi
}
...
clean_xcode() {
    ...
    if [[ -d "$old_archives" ]]; then
        ...
        if [[ $count -gt 0 ]]; then
            ...
            if [[ "$DRY_RUN" == true ]]; then
                print_color "$CYAN" "  [DRY RUN] Would clean: $count old Xcode archives (>30 days)"
            else
                find "$old_archives" -type d -mtime +30 -exec rm -rf {} \; 2>/dev/null
                print_color "$GREEN" "  ✓ Cleaned $count old Xcode archives"
            fi
            ((TOTAL_FREED += size))
        fi
    fi
    ...
    if command -v xcrun &> /dev/null; then
        if [[ "$DRY_RUN" != true ]]; then
            ...
            print_color "$GREEN" "  ✓ Removed unavailable simulators"
        else
            print_color "$CYAN" "  [DRY RUN] Would remove unavailable simulators"
        fi
    fi
}
        ...
        for backup in "$backup_dir"/*; do
            if [[ -d "$backup" ]]; then
                ...
                if [[ "$VERBOSE" == true ]]; then
                    print_color "$YELLOW" "  Found backup: $backup_name ($human_size)"
                fi
            fi
        done
        
        if [[ $count -gt 0 ]]; then
            print_color "$YELLOW" "  Found $count iOS backup(s) - Total: $(format_bytes $total_size)"
            print_color "$YELLOW" "  iOS backups are kept for safety. Remove manually if needed."
        else
            print_color "$GREEN" "  ✓ No iOS backups found"
        fi
    fi
}
...
clean_homebrew() {
    ...
    if command -v brew &> /dev/null; then
        ...
        if [[ "$DRY_RUN" == true ]]; then
            print_color "$CYAN" "  [DRY RUN] Would clean: Homebrew cache ($(format_bytes $cache_size))"
            ((TOTAL_FREED += cache_size))
        else
            brew cleanup -s 2>/dev/null
            brew cleanup --prune=all 2>/dev/null
            print_color "$GREEN" "  ✓ Cleaned Homebrew cache ($(format_bytes $cache_size))"
            ((TOTAL_FREED += cache_size))
        fi
    else
        [[ "$VERBOSE" == true ]] && print_color "$YELLOW" "  ⊘ Homebrew not installed"
    fi
}
...
clean_npm() {
    ...
    if command -v npm &> /dev/null; then
        ...
        if [[ "$DRY_RUN" == true ]]; then
            print_color "$CYAN" "  [DRY RUN] Would clean: npm cache ($(format_bytes $cache_size))"
            ((TOTAL_FREED += cache_size))
        else
            npm cache clean --force 2>/dev/null
            print_color "$GREEN" "  ✓ Cleaned npm cache ($(format_bytes $cache_size))"
            ((TOTAL_FREED += cache_size))
        fi
    else
        [[ "$VERBOSE" == true ]] && print_color "$YELLOW" "  ⊘ npm not installed"
    fi
}
...
clean_pip() {
    ...
    if command -v pip3 &> /dev/null || command -v pip &> /dev/null; then
        ...
        if [[ "$DRY_RUN" == true ]]; then
            print_color "$CYAN" "  [DRY RUN] Would clean: pip cache ($(format_bytes $cache_size))"
            ((TOTAL_FREED += cache_size))
        else
            ...
            print_color "$GREEN" "  ✓ Cleaned pip cache ($(format_bytes $cache_size))"
            ((TOTAL_FREED += cache_size))
        fi
    else
        [[ "$VERBOSE" == true ]] && print_color "$YELLOW" "  ⊘ pip not installed"
    fi
}
...
clean_gems() {
    ...
    if command -v gem &> /dev/null; then
        ...
        if [[ "$DRY_RUN" == true ]]; then
            print_color "$CYAN" "  [DRY RUN] Would clean: gem cache ($(format_bytes $cache_size))"
            ((TOTAL_FREED += cache_size))
        else
            gem cleanup 2>/dev/null
            print_color "$GREEN" "  ✓ Cleaned gem cache"
            ((TOTAL_FREED += cache_size))
        fi
    else
        [[ "$VERBOSE" == true ]] && print_color "$YELLOW" "  ⊘ Ruby gems not installed"
    fi
}
...
clean_docker() {
    ...
    if command -v docker &> /dev/null; then
        if [[ "$DRY_RUN" == true ]]; then
            print_color "$CYAN" "  [DRY RUN] Would clean: Docker system"
        else
            docker system prune -af --volumes 2>/dev/null
            print_color "$GREEN" "  ✓ Cleaned Docker system"
        fi
    else
        [[ "$VERBOSE" == true ]] && print_color "$YELLOW" "  ⊘ Docker not installed"
    fi
}
...
clean_system_cache() {
    ...
    if [[ -d "$system_cache" ]]; then
        ...
        print_color "$YELLOW" "  System cache: $(format_bytes $size) (requires admin password)"
        ...
        if [[ "$DRY_RUN" != true ]]; then
            ...
            if [[ $REPLY =~ ^[Yy]$ ]]; then
                ...
                print_color "$GREEN" "  ✓ Cleaned system cache"
                ((TOTAL_FREED += size))
            fi
        else
            print_color "$CYAN" "  [DRY RUN] Would clean system cache"
            ((TOTAL_FREED += size))
        fi
    fi
}
...
clean_logs() {
    ...
    if [[ "$DRY_RUN" != true ]]; then
        ...
        print_color "$GREEN" "  ✓ Cleaned logs older than 30 days"
    else
        print_color "$CYAN" "  [DRY RUN] Would clean logs older than 30 days"
    fi
...
clean_downloads() {
    ...
    if [[ $old_count -gt 0 ]]; then
        ...
        print_color "$YELLOW" "  Found $old_count files older than 30 days ($(format_bytes $old_size))"
        ...
        if [[ "$DRY_RUN" != true ]]; then
            ...
            if [[ $REPLY =~ ^[Yy]$ ]]; then
                ...
                print_color "$GREEN" "  ✓ Moved old downloads to trash"
                ((TOTAL_FREED += old_size))
            fi
        else
            print_color "$CYAN" "  [DRY RUN] Would move old downloads to trash"
            ((TOTAL_FREED += old_size))
        fi
    else
        print_color "$GREEN" "  ✓ No old downloads found"
    fi
}
...
clean_trash() {
    ...
    if [[ $trash_size -gt 0 ]]; then
        if [[ "$DRY_RUN" == true ]]; then
            print_color "$CYAN" "  [DRY RUN] Would empty trash ($(format_bytes $trash_size))"
            ((TOTAL_FREED += trash_size))
        else
            rm -rf "$HOME/.Trash/"* 2>/dev/null
            print_color "$GREEN" "  ✓ Emptied trash ($(format_bytes $trash_size))"
            ((TOTAL_FREED += trash_size))
        fi
    else
        print_color "$GREEN" "  ✓ Trash is already empty"
    fi
}
...
analyze_junk() {
    print_color "$BLUE" "═══════════════════════════════════════════"
    print_color "$BLUE" "Analyzing System Junk..."
    print_color "$BLUE" "═══════════════════════════════════════════"
    ...
    [[ $xcode_size -gt 0 ]] && print_color "$CYAN" "Xcode junk: $(format_bytes $xcode_size)"
    ((total += xcode_size))
    
    # iOS Backups
    local ios_size=$(get_size "$HOME/Library/Application Support/MobileSync/Backup")
    [[ $ios_size -gt 0 ]] && print_color "$CYAN" "iOS backups: $(format_bytes $ios_size)"
    ((total += ios_size))
    ...
    [[ $cache_size -gt 0 ]] && print_color "$CYAN" "Cache files: $(format_bytes $cache_size)"
    ((total += cache_size))
    
    # Trash
    local trash_size=$(get_size "$HOME/.Trash")
    [[ $trash_size -gt 0 ]] && print_color "$CYAN" "Trash: $(format_bytes $trash_size)"
    ((total += trash_size))
    
    print_color "$BLUE" "═══════════════════════════════════════════"
    print_color "$YELLOW" "Total junk found: $(format_bytes $total)"
...
main() {
    ...
    print_color "$BLUE" "═══════════════════════════════════════════"
    print_color "$BLUE" "Mac Power Tools - System Junk Cleaner"
    ...
    if [[ "$DRY_RUN" == true ]]; then
        ...
        print_color "$GREEN" "✓ Total space freed: $(format_bytes $TOTAL_FREED)"
    fi
    print_color "$BLUE" "═══════════════════════════════════════════"
}
