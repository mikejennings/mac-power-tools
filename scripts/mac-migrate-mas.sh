...
# Mac Power Tools - Migrate from Mac App Store to Homebrew
# Helps users migrate apps from Mac App Store to Homebrew Cask versions
...
migrate_app() {
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        print_color "$CYAN" "  - Uninstall MAS version (ID: $mas_id)"
    ...
    if brew install --cask "$cask_name"; then
        ...
        print_color "$YELLOW" "Removing Mac App Store version..."
        if mas uninstall "$mas_id" 2>/dev/null; then
            print_color "$GREEN" "✓ Removed MAS version"
        else
            print_color "$YELLOW" "Note: Could not remove MAS version automatically"
            print_color "$CYAN" "You may need to manually delete it from /Applications"
        fi
...
analyze_migration() {
    print_color "$BLUE" "Analyzing Mac App Store apps for migration opportunities..."
    echo
...
add_custom_mapping() {
    ...
    print_color "$CYAN" "Adding custom mapping: $mas_id → $cask_name"
...
list_mappings() {
    print_color "$BLUE" "Known Mac App Store to Homebrew Cask mappings:"
    print_color "$BLUE" "═══════════════════════════════════════════"
...
main() {
        ...
        case $1 in
            ...
            -m|--map)
                if [ -n "$2" ] && [ -n "$3" ]; then
                    ...
                    print_color "$RED" "Error: --map requires MAS_ID and CASK_NAME"
                    exit 1
                fi
                ;;
            -v|--verbose)
    ...
    print_color "$BLUE" "═══════════════════════════════════════════"
    print_color "$BLUE" "Mac App Store to Homebrew Migration Tool"
    print_color "$BLUE" "═══════════════════════════════════════════"
    echo
    ...
    if [ ${#migratable_apps[@]} -eq 0 ] || [ -z "${migratable_apps[0]}" ]; then
        ...
        print_color "$CYAN" "All your Mac App Store apps are either:"
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        print_color "$YELLOW" "⚠️  This will modify your installed applications"
    fi
