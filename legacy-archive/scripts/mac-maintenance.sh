...
# Mac Maintenance Script - System maintenance utilities
# Replaces mac-cli maintenance functionality
...
empty_trash() {
    ...
    print_status "Current trash size: $trash_size"
...
clear_caches() {
    ...
    cache_size=$(du -sh ~/Library/Caches 2>/dev/null | cut -f1 || echo "0B")
    print_status "Current user cache size: $cache_size"
    
    read -p "Clear user caches? (y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    read -p "Clear system caches (requires sudo)? (y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
...
clean_downloads() {
    print_status "Analyzing Downloads folder..."
    
    downloads_size=$(du -sh ~/Downloads 2>/dev/null | cut -f1 || echo "0B")
    print_status "Downloads folder size: $downloads_size"
    ...
    if [ -f ~/Library/Scripts/Folder\ Action\ Scripts/Sort\ Downloads\ by\ Date\ and\ Type.scpt ]; then
        ...
        echo "Run scripts/sort-downloads.sh to organize Downloads folder"
    fi
}
...
find_large_directories() {
    print_status "Finding large directories..."
...
clean_logs() {
    ...
    # User logs
    user_logs_size=$(du -sh ~/Library/Logs 2>/dev/null | cut -f1 || echo "0B")
    print_status "User logs size: $user_logs_size"
    
    read -p "Clean old user logs (>30 days)? (y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    # System logs
    read -p "Clean old system logs (requires sudo, >30 days)? (y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
...
rebuild_spotlight() {
    print_status "Rebuilding Spotlight index..."
    
    read -p "Rebuild Spotlight index? This may take a while. (y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        sudo mdutil -E /
...
# Toggle hidden files
toggle_hidden_files() {
    current_state=$(defaults read com.apple.finder AppleShowAllFiles 2>/dev/null || echo "NO")
    
    if [ "$current_state" = "YES" ]; then
        print_status "Hiding hidden files..."
        defaults write com.apple.finder AppleShowAllFiles NO
        print_success "Hidden files are now hidden"
    else
        print_status "Showing hidden files..."
...
repair_permissions() {
    ...
    read -p "Run disk First Aid (requires sudo)? (y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
...
# Kill all apps
kill_all_apps() {
    print_status "This will close all open applications..."
    
    read -p "Close all applications? (y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
...
# System sleep
system_sleep() {
    print_status "Putting system to sleep in 5 seconds..."
...
# System restart
system_restart() {
    print_status "System will restart in 10 seconds..."
    echo "Press Ctrl+C to cancel"
    sleep 10
    sudo shutdown -r now
}
...
# System shutdown
system_shutdown() {
    print_status "System will shutdown in 10 seconds..."
    echo "Press Ctrl+C to cancel"
    sleep 10
    sudo shutdown -h now
}
