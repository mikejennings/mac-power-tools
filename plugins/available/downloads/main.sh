...
sort_downloads() {
    ...
    print_info "Sorting downloads folder..."
    ...
    if [[ $count -gt 0 ]]; then
        print_success "✓ Sorted $count files"
        log_message "Batch sort: $count files sorted"
    else
        print_warning "No files to sort"
    fi
    
    if [[ $errors -gt 0 ]]; then
        print_error "⚠ $errors files failed to sort"
    fi
}

# Function to create and install Folder Action
install_folder_action() {
    print_info "Installing Folder Action for automatic sorting..."
...
EOF
    ...
    if [[ $? -eq 0 ]]; then
        print_success "✓ Folder Action installed successfully"
        log_message "Folder Action installed"
    else
        print_error "Failed to install Folder Action"
        return 1
    fi
}
...
install_launchd_agent() {
    print_info "Installing launchd agent for periodic sorting..."
...
EOF
    ...
    if launchctl load "$LAUNCHD_PLIST" 2>/dev/null; then
        print_success "✓ Launchd agent installed successfully"
        log_message "Launchd agent installed"
    else
        print_warning "Could not install launchd agent (may need Full Disk Access)"
    fi
}
...
show_status() {
    print_info "═══════════════════════════════════════════"
    print_info "Downloads Management Status"
    print_info "═══════════════════════════════════════════"
    ...
    if [[ "$folder_actions_enabled" == "true" ]]; then
        print_success "✓ Folder Actions: Enabled"
        ...
        if [[ $has_action -gt 0 ]]; then
            print_success "  ✓ Downloads folder action: Active"
        else
            print_warning "  ⚠ Downloads folder action: Not attached"
        fi
    else
        print_error "✗ Folder Actions: Disabled"
    fi
    
    # Check launchd agent
    if launchctl list | grep -q "com.mac-power-tools.downloads-sort"; then
        print_success "✓ Launchd agent: Running"
    else
        print_warning "⚠ Launchd agent: Not running"
    fi
    
    # Check Downloads folder
    echo
    print_info "Downloads Folder Statistics:"
    
    # Count files by type
    local total_files=$(find "$DOWNLOADS_DIR" -maxdepth 1 -type f | wc -l | tr -d ' ')
    print_info "  Unsorted files: $total_files"
    
    # Count sorted folders
    local date_folders=$(find "$DOWNLOADS_DIR" -maxdepth 1 -type d -name "????-??-??" | wc -l | tr -d ' ')
    print_info "  Date folders: $date_folders"
    
    # Show disk usage
    local folder_size=$(du -sh "$DOWNLOADS_DIR" 2>/dev/null | cut -f1)
    print_info "  Total size: $folder_size"
...
watch_downloads() {
    print_info "Watching Downloads folder for changes..."
    print_warning "Press Ctrl+C to stop"
    
    # Check if fswatch is installed
    if command -v fswatch >/dev/null 2>&1; then
        print_info "Using fswatch for monitoring..."
        ...
        print_info "Install fswatch for better performance: brew install fswatch"
...
analyze_downloads() {
    print_info "═══════════════════════════════════════════"
    print_info "Downloads Folder Analysis"
    print_info "═══════════════════════════════════════════"
    
    # File type distribution
    print_info "File Type Distribution:"
...
clean_old_downloads() {
    ...
    print_info "Finding downloads older than $days days..."
    ...
    print_warning "Found $count files (${total_size}MB) older than $days days"
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        print_success "✓ Deleted $count old files"
    ...
    case "${1:-}" in
        ...
        sort-file)
            if [[ -n "${2:-}" ]]; then
                ...
                print_error "Error: Please provide a file path"
                exit 1
            fi
            ;;
        setup)
