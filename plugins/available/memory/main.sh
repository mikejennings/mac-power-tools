...
display_memory_info() {
    ...
    if (( $(echo "$MEMORY_PRESSURE < 50" | bc -l) )); then
        print_success "${MEMORY_PRESSURE}% (Low)"
    elif (( $(echo "$MEMORY_PRESSURE < 75" | bc -l) )); then
        print_warning "${MEMORY_PRESSURE}% (Medium)"
    else
        print_error "${MEMORY_PRESSURE}% (High)"
    fi
    
    echo
    print_info "━━━ Memory Breakdown ━━━"
...
get_top_memory_apps() {
    ...
    echo
    print_info "━━━ Top $count Memory Consumers ━━━"
...
purge_memory() {
    echo
    print_warning "⚡ Purging inactive memory..."
    ...
    if sudo purge 2>/dev/null; then
        ...
        if [[ $freed -gt 0 ]]; then
            print_success "✓ Successfully purged $(format_bytes $freed) of memory"
        else
            print_success "✓ Memory purged (already optimized)"
        fi
    else
        print_error "✗ Failed to purge memory (requires admin password)"
    fi
}
...
kill_memory_hogs() {
    ...
    echo
    print_warning "🎯 Finding processes using more than ${threshold}% memory..."
    
    local hogs=$(ps aux | awk -v threshold="$threshold" '$4 > threshold {print $2, $4, $11}' | \
                 grep -v "PID" | sort -rnk 2)
    
    if [[ -z "$hogs" ]]; then
        print_success "✓ No processes using more than ${threshold}% memory"
        return
    fi
    
    print_warning "Found memory-intensive processes:"
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        echo "$hogs" | while read pid mem cmd; do
            if kill -9 "$pid" 2>/dev/null; then
                print_success "  ✓ Killed: $cmd (PID: $pid)"
            else
                print_error "  ✗ Failed to kill: $cmd (PID: $pid)"
            fi
        done
    fi
}
...
optimize_memory() {
    ...
    echo
    print_info "Current memory pressure: ${MEMORY_PRESSURE}%"
    
    # Optimization steps
    echo
    print_warning "1. Closing unused applications..."
        ...
        echo "$inactive_apps" | tr ',' '\n' | while read -r app; do
            ...
            if [[ -n "$app" ]]; then
                osascript -e "tell application \"$app\" to quit" 2>/dev/null && \
                    print_success "  ✓ Closed: $app"
            fi
        done
    fi
    
    echo
    print_warning "2. Purging inactive memory..."
    purge_memory
    
    echo
    print_warning "3. Clearing caches..."
    # Clear user cache safely
    rm -rf "$HOME/Library/Caches/"* 2>/dev/null
    print_success "  ✓ Cleared user caches"
    
    # Final status
    get_memory_info
    echo
    print_success "✓ Optimization complete!"
    print_info "New memory pressure: ${MEMORY_PRESSURE}%"
    
    local freed=$((before_available - MEMORY_AVAILABLE))
    if [[ $freed -gt 0 ]]; then
        print_success "Total memory freed: $(format_bytes $freed)"
    fi
}
...
monitor_memory() {
    ...
    print_info "Memory Monitor (updating every ${interval}s, Ctrl+C to stop)"
    print_info "═══════════════════════════════════════════"
    ...
    while true; do
        ...
        if (( $(echo "$MEMORY_PRESSURE > $THRESHOLD" | bc -l) )); then
            echo
            print_error "⚠️  HIGH MEMORY PRESSURE DETECTED!"
            print_warning "Run 'mac memory --optimize' to free up memory"
        fi
...
show_system_memory_info() {
    print_info "═══════════════════════════════════════════"
    print_info "System Memory Information"
    print_info "═══════════════════════════════════════════"
